...
| `data/` | Raw GeoTIFFs — held as [GitHub Release assets](https://github.com/andreasalem/switzerland-flooding-gee/releases/tag/data-2026-08-27), provenance in [`data/MANIFEST.md`](data/MANIFEST.md) |
| `assets/` | Pre-styled PNG/JPEG overlays the pages actually render (1–2 MB each), generated by `render_overlays.py` |
| `render_overlays.py` | Warps the raw TIFFs to EPSG:3857 and bakes the display styling into small web overlays |
...
- (2026-08-27) Both previous TODOs are resolved: the Blatten before-image was re-exported
  with the summer-2024 window (`ch_blatten_before_summer2024.tif`, see the manifest), and
  the pages now load 1–2 MB pre-rendered overlays from `assets/` instead of 14–18 MB
  GeoTIFFs — page weight dropped roughly tenfold.
