...
**Frontend stack**: Leaflet.js + vendored leaflet-side-by-side + `L.imageOverlay`.
Since 2026-08-27 the pages render pre-styled PNG/JPEG overlays (1–2 MB each) generated by
`render_overlays.py` (EPSG:3857 warp + styling baked in); the in-browser GeoTIFF parsing
stack (georaster-layer-for-leaflet, 28–35 MB per page) is retired.

**Data storage**: raw GeoTIFFs are GitHub Release assets (`data-2026-08-27`); the git tree
carries only `data/MANIFEST.md` and the small `assets/` overlays.
